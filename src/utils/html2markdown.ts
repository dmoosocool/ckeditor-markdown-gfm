...
turndownService.use( [ gfm, todoList, Katex ] );
...
function Katex( turndownService: TurndownService ): void {
	turndownService.addRule( 'katexBlock', {
		filter( node: any ) {
			return node.nodeName === 'SCRIPT' && node.type === 'math/tex; mode=display';
		},
		replacement( content: any, node: any ) {
			const leftDelimiter = '$$\n';
			const rightDelimiter = '\n$$';
			return leftDelimiter + node.innerHTML + rightDelimiter;
		}
	} );

	turndownService.addRule( 'katexInline', {
		filter( node: any ) {
			return node.nodeName === 'SCRIPT' && node.type === 'math/tex';
		},
		replacement( content: any, node: any ) {
			const leftDelimiter = '$';
			const rightDelimiter = '$';
