...
export default class PasteFromMarkdownExperimental extends Plugin {
	...
	public init(): void {
		...
		this.listenTo<ClipboardInputTransformationEvent>(
			...
			( evt, data ) => {
				...
				if ( !dataAsTextHtml ) {
					const dataAsTextPlain =
						data.dataTransfer.getData( 'text/plain' );

					data.content =
						this._gfmDataProcessor.toView( dataAsTextPlain );

					return;
				}

				const markdownFromHtml =
					this._parseMarkdownFromHtml( dataAsTextHtml );

				if ( markdownFromHtml ) {
					data.content =
						this._gfmDataProcessor.toView( markdownFromHtml );
				}
			}
		);
	}
	...
	private _parseMarkdownFromHtml( htmlString: string ): string | null {
		...
		const withoutWrapperTag = this._removeFirstLevelWrapperTagsAndBrs(
			withoutOsSpecificTags
		);
		...
		return this._replaceHtmlReservedEntitiesWithCharacters(
			withoutWrapperTag
		);
	}
	...
	private _removeOsSpecificTags( htmlString: string ): string {
		...
		const withoutHtmlTag = withoutMetaTag
			.replace( /^<html>/, '' )
			.replace( /<\/html>$/, '' )
			.trim();
		// Removing the <body> tag present on Windows.
		const withoutBodyTag = withoutHtmlTag
			.replace( /^<body>/, '' )
			.replace( /<\/body>$/, '' )
			.trim();

		// Removing the <!--StartFragment--> tag present on Windows.
		return withoutBodyTag
	...
	private _containsOnlyAllowedFirstLevelTags( htmlString: string ): boolean {
		...
		const { body: tempElement } = parser.parseFromString(
			htmlString,
			'text/html'
		);

		const tagNames = Array.from( tempElement.children ).map(
			el => el.tagName
		);

		return tagNames.every( el =>
			ALLOWED_MARKDOWN_FIRST_LEVEL_TAGS.includes( el )
		);
	}
